...
export default function Header({ open, onOpenMenu }) {
  ...
  const [farmSelected, setFarmSelected] = React.useState(farm[0]);
  const [openSelect, setOpenSelect] = React.useState(false);

  let isTabletMid = useMediaQuery({ query: "(max-width: 768px)" }); // this is not defined
  return (
    <div className='flex flex-row w-full h-12 border-b px-1 justify-between items-center bg-white'>
      ...
      <div className='w-full flex flex-row justify-between items-center '>
        <Dropdown data={farm} farmSelected={farmSelected} setFarmSelected={setFarmSelected}/>
