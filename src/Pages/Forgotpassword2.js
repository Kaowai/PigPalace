...
function Forgotpassword2() {
    ...
    return (
        <div
            className='w-full h-full flex animate-slide-in-from-right flex-col gap-5 py-8'
        >
            ...
            <div className='flex flex-col gap-3 w-full h-32 items-center justify-start'>
                <div className='w-full items-center' >
                    <InputPIN arrayPIN={arrayPIN} setArrayPIN={setArrayPIN} code={code}/>
                </div>
            </div>
            <div className="w-[22rem] h-12 items-center flex justify-center">
                <button
                    className={`w-16 h-16 rounded-2xl flex items-center justify-center font-medium text-xs outline-none border-2 border-login ${arrayPIN.join('').length === 6 ? 'bg-primary_main text-white cursor-pointer button-hover' : 'bg-white text-slate-300 cursor-not-allowed'}`}
                    onClick={handleSubmit}>
                    <FaArrowRight size={24} />
                </button>
            </div>
            <div className='w-full flex items-center justify-center'>
                <span className='text-textdisable font-normal text-xs text-center hover:text-primary_main cursor-pointer' onClick={handleSendCode}>Send code again</span>
            </div>
